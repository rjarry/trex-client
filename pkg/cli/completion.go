...
// optional one-line help text. File candidates carry type hints so a renderer
// can colour them like ls (honouring LS_COLORS).
type Candidate struct {
	Value string
	Help  string
	Tag   string // grouping label: command, flag, value or file
	IsDir bool
	IsExe bool
}
...
func bashComplete(app *kong.Kong) bool {
	...
	// Drop file candidates: bash falls back to readline's native filename
	// completion (see "complete -o default" in etc/trex.bash-completion),
	// which handles trailing spaces, escaping and colours natively.
...
func completeArgs(app *kong.Kong, words []string, trailing bool) ([]Candidate, string) {
	...
	addCand := func(c Candidate) {
		if !strings.HasPrefix(c.Value, current) || seen[c.Value] {
			return
		}
		candidates = append(candidates, c)
	}
	add := func(value, help, tag string) {
		addCand(Candidate{Value: value, Help: help, Tag: tag})
	}
	...
	for _, child := range node.Children {
		...
		add(child.Name, child.Help, "command")
	}
	...
	for n := node; n != nil; n = n.Parent {
		for _, flag := range n.Flags {
			...
			add(long, flag.Help, "flag")
			if flag.Tag.Negatable != "" {
				add("--no-"+flag.Name, flag.Help, "flag")
			}
		}
	}
...
func fileCompletions(prefix string) []Candidate {
	...
	for _, e := range entries {
		...
		out = append(out, Candidate{Value: value, Tag: "file", IsDir: e.IsDir(), IsExe: exe})
	}
	return out
}
