...
func buildVM(entries []dslVMEntry, pkt *stl.Packet) (stl.VM, error) {
	...
	for i, e := range entries {
		...
		for kind, f := range e {
			if kind == "checksum_sweep" {
				more, err := buildChecksumSweep(f, pkt)
				if err != nil {
					return vm, fmt.Errorf("vm instruction %d (%s): %w", i, kind, err)
				}
				vm.Instructions = append(vm.Instructions, more...)
				continue
			}
...
// buildChecksumSweep expands the "checksum_sweep" sugar into a pair of lockstep
// flow variables that spread traffic over a swept 16-bit field (typically the
// outer UDP source port) while keeping a one's-complement checksum valid without
// hardware offload. Incrementing the field by one decrements the checksum by
// one, so a decrementing variable written to the checksum cancels it out. The
// checksum's current value is read from the built packet, so the sweep is sized
// to stop before the end-around carry would wrap it through zero.
func buildChecksumSweep(f map[string]any, pkt *stl.Packet) ([]stl.VMInstruction, error) {
	off, err := resolveOffset(pkt, f["offset"])
	if err != nil {
		return nil, err
	}
	csum, err := resolveOffset(pkt, f["checksum"])
	if err != nil {
		return nil, err
	}
	if csum+2 > len(pkt.Binary) {
		return nil, fmt.Errorf("checksum offset %d out of range", csum)
	}
	base := uint64(binary.BigEndian.Uint16(pkt.Binary[csum : csum+2]))
	if base < 2 {
		return nil, fmt.Errorf("checksum %d too small to sweep", base)
	}
	span := base - 1
	min := uint64(1024)
	if v, ok := f["min"]; ok {
	...
	return []stl.VMInstruction{
		stl.NewFlowVar("sport", 2, stl.OpInc, min, min, min+span, 1, false),
		stl.NewWriteFlowVar("sport", off, 0, true),
		stl.NewFlowVar("ocks", 2, stl.OpDec, base, base-span, base, 1, false),
		stl.NewWriteFlowVar("ocks", csum, 0, true),
	}, nil
}
